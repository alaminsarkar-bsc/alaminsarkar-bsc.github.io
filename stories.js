// ====================================
// FILE: stories.js
// বিবরণ: স্টোরি তৈরি, আপলোড, রেন্ডারিং এবং ভিউয়ার লজিক
// ====================================
...
function toggleMute() {
    ...
    if(storyEditorState.isMuted) { btn.innerHTML = '<i class="fas fa-microphone-slash" style="color:red;"></i>'; } 
...
function resetEditorState() {
    storyEditorState = { 
        mode: 'text', mediaFile: null, mediaBlob: null, 
        bgColor: STORY_GRADIENTS[0], isRecording: false, isMuted: false, 
        recordingTimer: null, mediaRecorder: null, recordedChunks: [], 
        stream: null, maxDuration: 30 
    };
...
async function publishProStory() {
    ...
    try {
        ...
        if (storyEditorState.mode === 'text') {
            ...
            // html2canvas is assumed to be loaded globally via CDN in HTML
