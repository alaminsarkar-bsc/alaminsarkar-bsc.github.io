...
async function initHomePage() {
    ...
    // স্টোরি লোড করা (ফাংশনটি stories.js এ থাকবে, যদি লোড হয়ে থাকে কল হবে)
    ...
    try {
        ...
        if (isProfilePage && status === 'saved') {
            if (savedPostIds.size === 0 && currentUser) {
                 // fetchSavedPostIds() auth.js এ আছে, গ্লোবাল হলে এখানে কাজ করবে
    ...
    card.addEventListener('click', (e) => {
        ...
        if (tapLength < 300 && tapLength > 0) {
            ...
            if (!loveBtn.classList.contains('loved')) {
                // handleReaction (interactions.js) needs to be global
...
function setupShortsObserver() {
    ...
    const options = { root: document.querySelector('.shorts-feed-container'), threshold: 0.6 };
    ...
    document.querySelectorAll('.short-card').forEach(card => { shortsObserver.observe(card); });
}
...
async function handlePrayerChange(payload) {
    ...
    if (eventType === 'INSERT') { 
        ...
        if (updatedPrayer) {
            ...
            if (existingCard) {
                // Update UI elements (counters etc.) logic is same as before
